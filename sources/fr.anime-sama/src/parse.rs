use aidoku::{
    Chapter, ContentRating, Manga, MangaPageResult, MangaStatus, Result, UpdateStrategy, Viewer,
...
fn extract_chapter(els: &Element) -> Chapter {
    let chapter_cover: Option<String> = els
        .select_first("a > .card-image-container > img")
        .and_then(|img| img.attr("src"));

    let chapter_url: Option<String> = None;
...
pub fn get_all_releases(response: &str, current_page: i32) -> Result<MangaPageResult> {
    ...
    let sorties = document
        .select("#list_catalog .catalog-card.card-base")
        .ok_or(HtmlError::NoResult)?;

    for els in sorties {
        let key = els
            .select_first("a > .card-content > h2")
            .and_then(|t| t.own_text())
            .unwrap_or_default();
    ...
    let last_page: Option<i32> = document
        .select("#list_pagination a") // returns ElementList
        .iter()
        .next()
